...
def make_action(screen, info, step, env, prev_action, highest_map_x_position):
    ...
    if PRINT_LOCATIONS:
        ...
        """
        print(
            "Mario's location in world:",
            mario_world_x,
            mario_world_y,
            f"({mario_status} mario)",
        )
        """
    ...
    # TODO: Jump over a enemy

    if enemy_locations != []:
        for e in enemy_locations:
            if e[0][0] < mario_x:
                print("test")
            if (e[0][0] - mario_x) < 50 and (e[0][0] - mario_x) > 25:
                print(e[0][0] - mario_x)
                return actions["RIGHT_JUMP_RUN"], info["x_pos"]

    """
    if mario_x == previous_x_position:
        if jump_ticks == 0:
            jump_ticks = 50
        jump_ticks -= 1
        while jump_ticks > 0:
    ...
    """
    if highest_map_x_position == info["x_pos"]:
        return actions["RIGHT_JUMP_RUN"], info["x_pos"]
    return actions["RIGHT_RUN"], info["x_pos"]

    # TODO: Jump over a hole in the ground
    # TODO: Prediction function for landing on a hazard
...
# obs - The screens pixels
# reward - The reward Mario has accumulated so far (from moving right)
# done - True if the game is over, False otherwise
# info - A dictionary containing information about the game (see below)
# step - The number of steps completed so far (including this one)
# action - The action chosen by your agent (this is what you need to change)
...
for step in range(100000):
    if obs is not None:
        action, previous_map_x_position = make_action(
            obs, info, step, env, action, highest_map_x_position
        )
        if previous_map_x_position > highest_map_x_position:
            highest_map_x_position = previous_map_x_position
        # print(highest_map_x_position)
    else:
